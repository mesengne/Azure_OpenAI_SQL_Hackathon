...
$DefaultValue = "SQLOAIHACK-SHARED"
...
$DefaultValue = "SQLOAIHACK-TEAM_VMs"
...
DownloadWithRetry "https://code.visualstudio.com/sha/download?build=stable&os=win32-x64" "$CopyPath\VSCode-Setup.exe" 10

DownloadWithRetry "https://github.com/git-for-windows/git/releases/download/v2.42.0.windows.2/Git-2.42.0.2-64-bit.exe" "$CopyPath\Github-Setup.exe" 10

DownloadWithRetry "https://www.python.org/ftp/python/3.10.11/python-3.10.11-amd64.exe" "$CopyPath\Python-Setup.exe" 10




##Distribute files from downlaod folder to local target folders
...
##Upload Files to build container to storage account
...
###################################################################
Write-Host -BackgroundColor Black -ForegroundColor Yellow "Creating DMS, Keyvault, storage account shared resources.................................................."
...
###################################################################
$SharedRG
$var= Get-AzResourceGroup -Name $SharedRG   #Gets the resource group details and stores in $var variable
$ResourceGroupID = $var.ResourceID   #Gets the id  from resource group properties and stores in $ResourceGroupID variable
Write-Host $ResourceGroupID

Write-Host -BackgroundColor Black -ForegroundColor Yellow "Creating Azure OpenAI Service.................................................."
$TemplateUri = (Join-Path $CurrentDir "ARM Templates\ARM Template - SQL Hackathon - OpenAI.json")
New-AzResourceGroupDeployment  -resourceGroupID $ResourceGroupID -resourceGroupName $SharedRG -TemplateUri $TemplateUri -name 'BuildOpenAI' -location $location -sku 'S0' -AsJob

<#
###################################################################
# DEPLOY OPENAI MODEL - WILL BE HANDLED LATER
###################################################################

New-AzCognitiveServicesAccount -ResourceGroupName  $SharedRG -Name OpenAI_Service -Type OpenAI -SkuName S0 -Location $Location
...
New-AzCognitiveServicesAccountDeployment -ResourceGroupName $SharedRG -AccountName OpenAI_Service -Name MyModel -Properties $properties -Sku $sku
...
Write-Warning "1. Restore databases for Monitoring labs by running the Launch_SQL_MI_configuration.ps1. Choose a remote TEAM VM. Note: Only run once."
Write-Warning "2. All labs and documentation can be found on TEAMVM's in C:\_SQLHACK_\LABS"